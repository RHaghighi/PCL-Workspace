...
/*
	...
	pass.setFilterLimits (min_dist-0.04, min_dist+0.1);
	pass.filter (*high_curvature_edges);
...
*/
	...
	//sor.filter (*high_curvature_edges);
  ...
  // Line Segmentation
	...
	extract.setNegative (false);
        ...
        //extract.filter (*high_curvature_edges);
        extract.filter (*segment_p1);


       std::cerr << "Model coefficients: " << line_coeff->values[0] << " " 
                                     << line_coeff->values[1] << " "
                                     << line_coeff->values[2] << " " 
                                     << line_coeff->values[3] << " " 
                                     << line_coeff->values[4] << " " 
                                     << line_coeff->values[5] << std::endl;
        std::stringstream ss;
        ss<<"line"<<k;
        int var=0;
        viewer.addLine (*line_coeff,ss.str(), var);
        viewer.setShapeRenderingProperties ( pcl::visualization::PCL_VISUALIZER_COLOR, 1, 0, 0,ss.str());
        viewer.setShapeRenderingProperties ( pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 7,ss.str());
